...
async function startNode() {
    ...
    try {
        ...
        console.log(`[Swarm] Cluster Node Active. PeerID: ${heliaNode.libp2p.peerId.toString()}`);
    ...
    try {
        ...
        if (data.type === 'identity') {
            ...
            if (data.cidn !== CLUSTER_CIDN) {
                console.warn(`[Swarm] Rejected peer ${peerId}: CIDN mismatch (${data.cidn})`);
                return;
            }
            ...
            if (isNewerVersion(data.version, BROWSER_VERSION)) {
                console.log(`[Swarm] Peer ${peerId} has newer version: ${data.version}. Sync required.`);
    ...
    if (heliaNode) {
        ...
        try {
            fs.writeFileSync(
                ...
                JSON.stringify([...following], null, 2), 'utf8'
            );
            fs.writeFileSync(
                path.join(IPFS_DATA_DIR, 'feed.json'),
                JSON.stringify(feed.slice(-100), null, 2), 'utf8' // Keep last 100 items
            );
            fs.writeFileSync(
                path.join(IPFS_DATA_DIR, 'pinned.json'),
                JSON.stringify(Object.fromEntries(pinnedFiles), null, 2), 'utf8'
            );
        } catch (e) {
...
function getStorageStats() {
    ...
    return {
        ...
        pinnedItems: [...pinnedFiles.entries()].map(([cid, meta]) => ({ cid, ...meta }))
    };
}
