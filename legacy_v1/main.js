...
function createWindow() {
  ...
  if (app.isPackaged) {
    // Production Mode: Load static Next.js export via electron-serve
    import('electron-serve').then((mod) => {
      const serve = mod.default;
      if (!loadURL) loadURL = serve({ directory: path.join(__dirname, 'next-shell', 'out') });
      loadURL(mainWindow);
    });
  } else {
...
function setupIPC() {
  ...
  // Wallet Reset (delete wallets.json to allow creating a new one)
