...
export default function ChatPageClient() {
  ...
  const [relationship, setRelationship] = useState<"sync" | "contact" | "none">(
    "none",
  );
  ...
  const fetchChat = async () => {
    try {
      // Fetch messages via IPC Local Node
      if (typeof window !== "undefined" && (window as any).bucksAPI) {
        const historyData = await (window as any).bucksAPI.chatHistory(peerId);
        setMessages(historyData.history || []);

        // Mark as read
        await (window as any).bucksAPI.chatMarkRead(peerId);
      }
      ...
      const peer = followResponse.data.following.find(
        (f: any) => f.peer_id === peerId,
      );
  ...
  useEffect(() => {
    ...
    // Listen for real-time incoming messages via IPC
    if (typeof window !== "undefined" && (window as any).bucksAPI) {
      (window as any).bucksAPI.onChatMessage((data: any) => {
    ...
    try {
      if (typeof window !== "undefined" && (window as any).bucksAPI) {
        if (selectedFile) {
          ...
          await (window as any).bucksAPI.chatSendFile(
            peerId,
            fileBytes,
            selectedFile.name,
          );
        } else {
          // Send text message via IPC
          await (window as any).bucksAPI.chatSend(peerId, newMessage);
        }
  ...
  const renderMedia = (msg: Message) => {
    ...
    const isPdf =
      msg.mime_type === "application/pdf" || /\.pdf$/i.test(msg.filename || "");
    ...
    if (isVideo) {
      return (
        <div className="mt-2 rounded-lg overflow-hidden border border-gray-100 bg-black">
          <video src={getIPFSUrl(cid)} controls className="max-w-full h-auto" />
        </div>
      );
    }
    ...
    <div className="flex flex-col h-screen max-w-2xl mx-auto bg-white border-x border-gray-100">
      ...
      <header className="flex items-center h-16 px-4 border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-10">
        ...
        <div className="flex items-center flex-1">
          ...
          <div>
            <h2 className="font-bold text-foreground leading-tight flex items-center space-x-2">
            ...
            <div className="flex items-center text-[10px] text-green-500 font-medium">
              <FaCircle className="text-[6px] mr-1" /> Online via Swarm
            </div>
          </div>
        </div>
      </header>
            ...
            return (
              ...
              >
                ...
                >
                  {renderMedia(msg)}
                  {msg.text && (
                    <p className={msg.cid ? "mt-2" : ""}>{msg.text}</p>
                  )}
                  <p
                    className={`text-[9px] mt-1 ${isMine ? "text-white/70" : "text-secondary opacity-70"}`}
                  >
