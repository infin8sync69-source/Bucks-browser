...
import { FaMessage, FaRegClock, FaUser, FaPlus, FaLock } from "react-icons/fa6";
...
export default function MessagesDashboard() {
  ...
  const fetchConversations = async () => {
    try {
      if (typeof window !== "undefined" && (window as any).bucksAPI) {
        const data = await (window as any).bucksAPI.chatConversations();
        ...
        const mappedConversations = data.conversations.map(
          (conv: Conversation) => {
            const peer = followingList.find(
              (f: any) => f.peer_id === conv.peer_id,
            );
            return {
              ...conv,
              username: peer
                ? peer.username
                : `Peer ${conv.peer_id.substring(0, 8)}...`,
            };
          },
        );
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined" && (window as any).bucksAPI) {
      (window as any).bucksAPI.onChatMessage(() => {
        fetchConversations();
      });
    }
  }, []);
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex flex-col sm:flex-row sm:items-center space-y-1 sm:space-y-0 sm:space-x-2 truncate">
                    <h3 className="font-semibold text-foreground truncate">
                      {conv.username}
                    </h3>
                    {(conv.encrypted || conv.hasSession) && <EncryptionBadge />}
                  </div>
