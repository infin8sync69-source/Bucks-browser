...
let chatEngine = null; // Lazy-loaded after IPFS is ready
...
function loadSettings() {
  ...
  if (!process.env.BUCKS_CLUSTER_SECRET)
    process.env.BUCKS_CLUSTER_SECRET = settings.clusterSecret;
  if (!process.env.BUCKS_CLUSTER_CIDN)
    process.env.BUCKS_CLUSTER_CIDN = settings.clusterCidn;
...
const COMPILED_AD_PATTERNS = AD_BLOCK_PATTERNS.map(
  (pattern) => new RegExp("^" + pattern.split("*").join(".*") + "$", "i"),
);
...
function setupIPC() {
  ...
  function isInternalOrigin(sender) {
    ...
    return (
      url.startsWith("file://") &&
      (url.includes("index.html") || url.includes("bucks-browser"))
    );
  }
  ...
  function validateOriginAccess(event) {
    ...
    if (origin.startsWith("file:") && !isInternalOrigin(sender))
      return "BLOCKED";
  ...
  function validateWalletSchema(params) {
    ...
    if (
      !ALLOWED_WALLET_METHODS.includes(method) ||
      !ALLOWED_WALLET_ENDPOINTS.includes(endpoint)
    ) {
      console.error(
        `[ATTACK SIGNAL] Invalid Endpoint/Method: ${method} ${endpoint}`,
      );
      return false;
    }
    ...
    if (method === "POST") {
      if (!body || typeof body !== "object") {
        console.error(
          `[ATTACK SIGNAL] Missing or invalid POST body for ${endpoint}`,
        );
        return false;
      }
      ...
      if (allowedKeys) {
        ...
        if (hasExtra) {
          console.error(
            `[ATTACK SIGNAL] Unauthorized keys detected in ${endpoint}: ${keys}`,
          );
          return false;
        }
      }
      ...
      console.error(
        `[ATTACK SIGNAL] Forbidden body detected in GET request to ${endpoint}`,
      );
      return false;
    }
  ...
  ipcMain.handle("wallet-rpc", async (event, params) => {
    ...
    if (access === "PENDING_APPROVAL") {
      ...
      mainWindow.webContents.send("wallet-access-request", {
        requestId,
        origin,
      });
    ...
    if (!validateWalletSchema(params)) {
      console.error(
        `[ATTACK SIGNAL] Blocked malformed wallet-rpc request: ${JSON.stringify(params)}`,
      );
  ...
  ipcMain.handle("social-rpc", async (event, params) => {
    ...
    if (!isInternalOrigin(event.sender))
    ...
    try {
      return new Promise((resolve) => {
        ...
        req.on("error", (err) =>
          resolve({ error: `Social Service Offline: ${err.message}` }),
        );
        if (body)
...
/* ─── E2E Encrypted Chat IPC Handlers ─── */
function setupChat() {
  ipcMain.handle("chat-send", async (_e, { peerId, text, attachmentCid }) => {
    if (!chatEngine) return { success: false, error: "Chat engine not ready" };
    return await chatEngine.sendMessage(peerId, text, attachmentCid);
  });

  ipcMain.handle("chat-history", async (_e, { peerId }) => {
    if (!chatEngine)
      return { history: [], sessionInfo: null, hasSession: false };
    return chatEngine.getChatHistory(peerId);
  });

  ipcMain.handle("chat-conversations", async () => {
    if (!chatEngine) return { conversations: [] };
    return chatEngine.getConversations();
  });

  ipcMain.handle("chat-mark-read", async (_e, { peerId }) => {
    if (!chatEngine) return { success: false };
    return chatEngine.markAsRead(peerId);
  });

  ipcMain.handle(
    "chat-send-file",
    async (_e, { peerId, fileData, filename }) => {
      if (!chatEngine)
        return { success: false, error: "Chat engine not ready" };
      return await chatEngine.sendFile(peerId, fileData, filename);
    },
  );

  ipcMain.handle("chat-key-bundle", async () => {
    if (!chatEngine) return null;
    return chatEngine.getOwnBundle();
  });

  ipcMain.handle("chat-process-bundle", async (_e, { peerId, bundle }) => {
    if (!chatEngine) return { success: false };
    return chatEngine.processPeerBundle(peerId, bundle);
  });
}
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    // 3. Initialize E2E encrypted chat engine
    chatEngine = require("./chat-engine");
    const heliaNode = ipfs.getHeliaNode();
    const gossipSub = ipfs.getGossip();
    const unixfs = ipfs.getFsModule();
    if (heliaNode && gossipSub) {
      await chatEngine.initChat(heliaNode, gossipSub, unixfs);
      // Forward incoming messages to renderer
      chatEngine.setOnMessageCallback((data) => {
        if (mainWindow && mainWindow.webContents) {
          mainWindow.webContents.send("chat-message", data);
        }
      });
      console.log("[App] Chat engine initialized.");
    }
  } catch (err) {
